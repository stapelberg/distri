	...
	for idx, fi := range fis {
		...
		if !simulate {
			...
			c.Log.Printf("stale package %s (got input_digest %q, want %q)",
				...
				meta.GetInputDigest(),
				inputDigest)
			if os.Getenv("DOIT") == "harder" && pkg != "fzf" && pkg != "steam" && pkg != "gcc-i686" {
				// TODO: make the package non-stale programmatically, i.e. ReadFile, strings.ReplaceAll, WriteFile
				b, err := ioutil.ReadFile(fn)
				if err != nil {
					return err
				}
				b = bytes.ReplaceAll(b, []byte(meta.GetInputDigest()), []byte(inputDigest))
				if err := ioutil.WriteFile(fn, b, 0644); err != nil {
					return err
				}
				log.Printf("replaced %s", fn)
			}
